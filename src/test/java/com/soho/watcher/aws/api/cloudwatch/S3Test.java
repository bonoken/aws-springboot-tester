...
public class S3Test {
	...
	//
	private String namespace = "AWS/S3";
	private String dimensionName = "BucketName";
	...
	public void getMetricStatistic(){
		...
		dimension.setValue("chatbot-op-serverlessdeploymentbucket-15ssk9boco1zr");
		
		List<Metric> awsMetricList = mon.listMetrics(reqMetric);
		
		logger.debug(awsMetricList);
		List<String> metricList = mon.getFilteredMetricsForDimension(awsMetricList , dimension);
		...
		for (String metricNm : metricList) {
			...
			if("BucketSizeBytes".equals(metricNm) || "NumberOfObjects".equals(metricNm)) {
				request.setPeriod(Integer.valueOf(108000));
				request.setStartTime(new Date(timeRangeDate[1].getTime() - (Integer.valueOf(108000).intValue() * 1000)));
				
				if("BucketSizeBytes".equals(metricNm)) {
					Dimension d = new Dimension();
					d.setName("StorageType");
					d.setValue("StandardStorage");
					dimensions.add(d);
				}
				
				if("NumberOfObjects".equals(metricNm)) {
					Dimension d = new Dimension();
					d.setName("StorageType");
					d.setValue("AllStorageTypes");
					dimensions.add(d);
				}
