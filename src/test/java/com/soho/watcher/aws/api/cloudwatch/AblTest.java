...
public class AblTest {
	...
	/** log */
	private static final Logger logger = LogManager.getLogger(AblTest.class);
	...
	//
	private String namespace = "AWS/ApplicationELB";
	private String dimensionName = "LoadBalancer";
	...
	public void getMetricStatistic(){
		...
		dimension.setValue("app/SEOUL-DEV-BESPIN-FRONT/284d6f130808cae1");
		...
		for (String metricNm : metricList) {
			...
			for(Datapoint datum :   mon.getMetricStatistics(request)) {
				if(null != datum) {
					...
					System.out.println("name : " + dName + ", val : " + dValue + ", metric name : " + metricNm);
